...
if jq -e '._embedded.mailboxes' $INBOXES_FILE >/dev/null; then
  ...
  for mailbox_id in $mailbox_ids; do
    ...
    if jq -e '._embedded.savedReplies' ${SAVED_REPLIES_FILE//"{mailboxId}"/$mailbox_id} >/dev/null; then
      saved_reply_ids=$(jq -r '._embedded.savedReplies[].id' ${SAVED_REPLIES_FILE//"{mailboxId}"/$mailbox_id})
